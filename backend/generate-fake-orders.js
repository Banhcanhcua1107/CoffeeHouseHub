    ...
    try {
        ...
        for (let i = 0; i < NUMBER_OF_ORDERS; i++) {
            ...
            if (rand < cumulative) { orderStatus = 'delivered'; paymentStatus = 'paid'; paymentMethod = Math.random() < 0.5 ? 'cod' : 'momo';
                ...
                } else { cumulative += STATUS_DISTRIBUTION.processing_pending;
                    if (rand < cumulative) { orderStatus = 'processing'; paymentStatus = 'pending'; paymentMethod = 'cod';
                    } else { cumulative += STATUS_DISTRIBUTION.shipped_pending;
                        if (rand < cumulative) { orderStatus = 'shipped'; paymentStatus = 'pending'; paymentMethod = 'cod';
